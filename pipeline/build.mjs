...
// A colectivo "line" is its NUMBER; the feed's route entries are its ramales.
// routes.txt holds 1052 of them — line 96 alone has 35, spelled 96AA…96AG, and
// others use the R-form (620R1…620R14). Keying on the leading digits collapses
// them to the 274 lines the city actually numbers, and the engine keeps the
// ramales as per-direction variants of that line. The 18 lettered services
// (AZUL1, ROJA, VERDE, NORTE11…) have no leading digits and keep their name.
const busKey = (sn) => {
  const s = (sn || '').trim();
  const m = /^(\d+)/.exec(s);
  return m ? m[1] : s;
...
// FOUR cfgs, THREE feeds — the richest map of the family:
//   colectivos  1052 route entries → 274 lines, road graph
//   subte       A–H, subway graph, official colours, metro treatment
//   Premetro    PM-C and PM-S, the light-rail feeder, tram graph
//   trenes      the seven suburban railways, own graph and own extract
// The metro treatment keys off cfg.metroCfg rather than the family's usual
// /^M[0-9]/ name test: Buenos Aires letters its metro A, B, C, D, E and H, and
// guessing from a name would either miss them or swallow a colectivo.
const MODES = [{
  mode: 'bus', label: 'colectivos', osmFile: 'data/osm/ba.json',
  graphMode: 'road', color: '#0059a9', colorDark: '#00294f',
  all: busAll, lines: busList.length ? busList : (busAll ? [] : ['60']),
  feeds: [
    { tag: 'col', dir: 'data/gtfs-bus', routeTypes: ['3'], mapKey: busKey },
  ],
}];
...
// of the Obelisk, Zárate 90 km north — so they get their own, much wider
// extract. Their 27 route entries are BRANCHES of seven lines ("Mitre /
// Retiro - Tigre", "Mitre / Villa Ballester - Zárate"), so the key is the part
// before the slash and the branches become variants, which is how the city
// itself names them.
// True when a line belongs to the subte cfg. The family's siblings ask the
// NAME whether it looks like a metro line, which cannot work for letters A–H,
// so the answer comes from the cfg instead: the metro cfg fills this set with
// its own keys and every downstream test reads it.
const METRO_KEYS = new Set();
const isMetro = (l) => METRO_KEYS.has(l);
const trenKey = (sn, r) => ((r && r.route_long_name) || sn || '').split('/')[0].trim();
  ...
  for (const feed of cfg.feeds) {
    ...
    for (const r of await readCsv(join(fdir, 'routes.txt'))) {
      ...
      if (r.route_type === '11') {
        ...
        // the subte's own line colours are the city's, and a metro without
        // them reads as one anonymous red system
    ...
    // EVERY shape is drawn, not just the busiest one per (line, direction).
    //
    // The siblings pick a single representative because a line there has one
    // route with the odd short-turn. In Buenos Aires the variants ARE the
    // network: a línea is a trunk that fans out into ramales — 96 has 35 of
    // them, 60 has 20 — and drawing only the busiest would show a third of the
    // city's coverage while looking complete. 2 066 shapes instead of 572
    // runs, under the same 292 line numbers, because a ramal is not a separate
    // line to a passenger: it is the same number with a different tail.
    const feedReps = [];
    for (const L of [...byLineDir.keys()].sort(numSort)) {
      const dirs = byLineDir.get(L);
      for (const dir of [...dirs.keys()].sort()) {
        const m = dirs.get(dir);
        // busiest first, so the trunk variant leads the line's list in meta
    ...
    log(`feed ${feed.tag}: ${feedReps.length} przebiegów z ${byLineDir.size} linii ` +
        `(wszystkie ramale, nie tylko najruchliwszy wariant)`);
