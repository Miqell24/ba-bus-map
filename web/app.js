...
async function init() {
  ...
  map.addControl(new maplibregl.AttributionControl({ compact: true, customAttribution: 'Timetables: Buenos Aires Ciudad — colectivos, SBASE, Trenes Argentinos' }));
  ...
  const nSub = rail.filter((l) => /^[A-EH]$/.test(l.line)).length;
  const nPre = rail.filter((l) => /^PM-/.test(l.line)).length;
  document.getElementById('count').textContent =
    `(${nBus} colectivos · ${nSub} subte · ${nPre} premetro · ${rail.length - nSub - nPre} trenes)`;
  ...
  async function exportBBox(bbox, btn, doneLabel, opts) {
    ...
    try {
      ...
      const txt = '© OpenStreetMap contributors · OpenFreeMap · GTFS: Buenos Aires Ciudad';
      ...
      a.download = `buenosaires-transit_${d.getFullYear()}-${pad(d.getMonth() + 1)}-${pad(d.getDate())}_${pad(d.getHours())}${pad(d.getMinutes())}_${out.width}x${out.height}.png`;
  ...
  btnAtlas.addEventListener('click', async () => {
    ...
    try {
      ...
      const a = document.createElement('a');
      a.download = `buenosaires-transit-giant_${d.getFullYear()}-${p2(d.getMonth() + 1)}-${p2(d.getDate())}_${Wf}x${Hf}.png`;
      a.href = URL.createObjectURL(blob);
  ...
  // NOT fitBounds(meta.bbox): the AMBA lines fan out 60 km from the Obelisk
  // and the railways run past Chascomús, so fitting the data bbox would open
  // on the pampas with the city as a speck. Open on central Buenos Aires.
  map.jumpTo({ center: [-58.4200, -34.6100], zoom: 11.2 });
}
